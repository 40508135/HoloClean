...
class RepairModel:
    ...
    def fit_model(self, X_train, Y_train, mask_train):
        ...
        print("learning rate is", self.env['learning_rate'])
        """
        ***
        CHANGE THE OPTIMISER HERE
        ***
        """
        optimizer = torch.optim.LBFGS(trainable_parameters, lr=self.env['learning_rate'])
        ...
        for i in tqdm(range(epochs)):
            ...
            def closure():
                optimizer.zero_grad()
                X_var = Variable(X_train[start:end], requires_grad=False)
                Y_var = Variable(Y_train[start:end], requires_grad=False)
                mask_var = Variable(mask_train[start:end], requires_grad=False)
                ...
                output = loss(fx, Y_var.squeeze(1))
                output.backward()
                return output
    ...
    def __train__(self, loss, optimizer, X_train, Y_train, mask_train):
        ...
        # Define the closure function required for L-BFGS optimization
        def closure():
            optimizer.zero_grad()
            fx = self.model.forward(X_var, index_var, mask_var)
            output = loss(fx, Y_var.squeeze(1))
